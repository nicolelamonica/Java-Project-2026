...
public class TestLoanClassTemplate {
  ...
  public static void extraPayFirstMonth(Loan l, double extraPay) {
	 ...
	 while (myLoanAmount >= myMonthlyPayment) {
            ...
            // Ensure extra payment is only applied the FIRST month
            extraPay = 0; 
        ...
        if (myLoanAmount < myMonthlyPayment && myLoanAmount > 0) {
            // calculate the new month's interest based on myLoanAmount and myMonthlyInterestRate
            interest = myLoanAmount * myMonthlyInterestRate;
            // The principal paid this month is exactly whatever is left of the loan
            principal = myLoanAmount;
            // calculate the new myLoanAmount after minus this month's principal (it will be 0)
  ...
  public static void extraPayEveryMonth(Loan l, double extraPay) {
	...
	while (myLoanAmount >= (myMonthlyPayment + extraPay)) {
        ...
        while (myLoanAmount >= myMonthlyPayment) {
            // section B2: Regular payments continue once the balance drops too low for the extra $1000
            interest = myLoanAmount * myMonthlyInterestRate;
            principal = myMonthlyPayment - interest;
            myLoanAmount = myLoanAmount - principal; 
            totalInterest = totalInterest + interest;
